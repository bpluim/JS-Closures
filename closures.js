...
that implement up/down counter. The first function is called inc, this function is responsible
...
function findPotentialFriends(existingFriends) {
  console.log('hit');
  return function(nonFriends) {
    console.log(nonFriends);
    for (var i = 0; i < existingFriends.length; i++) {
        if (existingFriends[i] === nonFriends) {
          return false;
        } else {
          return true;
        }
...
var potentialSecondLevelFriends = secondLevelFriends.filter(function(param) {
  return isNotAFriend(param);
});
var allPotentialFriends = allUsers.filter(function(param) {
  return isNotAFriend(param);
});
